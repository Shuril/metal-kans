    ...
    for layer in layers:
        ...
        for j in range(out_dim):
            ...
            for i in range(in_dim):
                ...
                # Evaluate single edge response
                y_resp = layer(X_test)[:, j]

                # Fit candidate functions via linear least squares: y = c * f(x)
                best_err = float("inf")
                best_edge = SymbolicEdge("linear", lambda x: x, "x", 0.0)

                for name, fn, c_expr in CANDIDATE_FUNCTIONS:
                    fx = fn(xs)
                    norm_fx = np.dot(fx, fx)
                    if norm_fx > 1e-8:
                        c = float(np.dot(fx, y_resp) / norm_fx)
                    else:
                        c = 0.0

                    diff = np.clip(y_resp - c * fx, -1e4, 1e4)
                    err = float(np.mean(diff ** 2))
                    if err < best_err:
                        best_err = err
                        best_edge = SymbolicEdge(name, fn, c_expr, c)

                row_edges.append(best_edge)
