...
from metal_kans import (
    ChebyKAN, FastKAN, ReLUKAN, BSplineKAN, MetalKAN,
    Adam, AdamW, Muon, Lion, RMSprop, SGD,
    CheckpointedMetalKAN, checkpoint_kan
)
...
class TestTraining(unittest.TestCase):
    ...
    def test_optimizers_reduce_loss(self):
        """Verify that AdamW, Muon, Lion, RMSprop, and SGD all successfully optimize MetalKAN."""
        X = np.linspace(-0.8, 0.8, 64).reshape(64, 1).astype(np.float32)
        y_true = np.sin(np.pi * X).astype(np.float32)

        opt_configs = [
            ("AdamW", lambda params: AdamW(params, lr=0.03, weight_decay=1e-4)),
            ("Muon", lambda params: Muon(params, lr=0.02, momentum=0.95)),
            ("Lion", lambda params: Lion(params, lr=0.01, weight_decay=1e-4)),
            ("RMSprop", lambda params: RMSprop(params, lr=0.02)),
            ("SGD_momentum", lambda params: SGD(params, lr=0.01, momentum=0.9, nesterov=True)),
        ]

        for name, opt_fn in opt_configs:
            np.random.seed(42)
            model = MetalKAN([1, 16, 1], basis_type="cheby", degree=4, bias=True, use_base=True)
            init_loss = float(np.mean((model(X) - y_true) ** 2))
            ...
            final_loss = float(np.mean((model(X) - y_true) ** 2))
            self.assertLess(
                final_loss,
                init_loss * 0.5,
                f"Optimizer {name} failed to sufficiently decrease loss: init={init_loss:.4f}, final={final_loss:.4f}"
            )

    def test_fused_gpu_train_step(self):
        """Verify fused monolithic GPU train_step for ChebyKAN, FastKAN, and BSplineKAN."""
        X = np.linspace(-0.8, 0.8, 64).reshape(64, 1).astype(np.float32)
        y_true = (X ** 2).astype(np.float32)

        models = [
            ("ChebyKAN", ChebyKAN(1, 1, degree=4, bias=True, use_base=True)),
            ("FastKAN", FastKAN(1, 1, num_centers=8, bias=True, use_base=True)),
            ("BSplineKAN", BSplineKAN(1, 1, grid_size=5, bias=True, use_base=True)),
        ]

        for name, model in models:
            y_init = model(X)
            init_loss = float(np.mean((y_init - y_true) ** 2))
            opt = AdamW(model.parameters(), lr=0.05, weight_decay=1e-4)

            # Test build_train_step
            step_fn = mk.build_train_step(model, opt, loss_fn="mse")
            for _ in range(50):
                loss_val = step_fn(X, y_true)

            y_final = model(X)
            final_loss = float(np.mean((y_final - y_true) ** 2))
            self.assertLess(
                final_loss,
                init_loss * 0.5,
                f"Fused train_step for {name} failed to decrease loss: init={init_loss:.4f}, final={final_loss:.4f}"
            )
